...
const enhance = compose(
  ...
  withHandlers({
    ...
    onPlacesChanged:
      ({ setPlaces, setLocation, refs, setMetadataMarker }) =>
      async (token, shop) => {
        ...
        let _meta = await AddressMetadataService.getAddressMetadataGraphql(
          locationRef.latitude,
    ...
    onMarkerChanged:
      ({ setLocation, setMetadataMarker }) =>
      async (locationRef, token, shop) => {
        setLocation(locationRef);
        let _meta = await AddressMetadataService.getAddressMetadataGraphql(
          locationRef.latitude,
          locationRef.longitude,
...
export default function withGoogleMaps(Component) {
  const WithGoogleMap = React.forwardRef((props, ref) => {
    const GoogleMapLayout = enhance((googleProps) => <Component {...props} ref={ref} googleProps={googleProps} />);
