...
function buildResult({ amount, fullName = null }) {
	...
	return {
		amount: floatAmount,
		data: { fullName },
		displayName: fullName ? `Pago en efectivo de ${fullName}` : null
	};
}

/**
 * @summary CashOnDeliveryForm component
 * @param {Object} props Props
 * @param {Object} ref Ref
 * @return {Object} React render
 */
function CashOnDeliveryForm(props, ref) {
	...
	if (!uniqueInstanceIdentifier) {
		setUniqueInstanceIdentifier(uniqueId("CashOnDeliveryForm"));
	}
...
// CashOnDeliveryForm to the wrapped component here, before setting the statics.
/* eslint-disable-next-line no-func-assign */
CashOnDeliveryForm = withComponents(forwardRef(CashOnDeliveryForm));
...
CashOnDeliveryForm.defaultProps = {
	isSaving: false,
	onChange() {},
	onReadyForSaveChange() {},
	onSubmit() {}
